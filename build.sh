#!/bin/bash
# ╔══════════════════════════════════════════════════════╗
# ║   OpenDarwin 1.0 - KURULUM EKRANI TAM ISO          ║
# ║   Plymouth Boot + Ubiquity + 5 Slayt + CSS         ║
# ╚══════════════════════════════════════════════════════╝
...
echo "║   Kurulum Ekranı TAM Sürüm          ║"
echo "╚══════════════════════════════════════╝"
echo -e "${N}"

# ── DİSK ──
DISK_FREE=$(df -BG /tmp | awk 'NR==2{print $4}' | sed 's/G//')
info "Disk: ${DISK_FREE}GB boş"
[ "$DISK_FREE" -lt 15 ] && { sudo apt clean; sudo rm -rf /tmp/* /var/cache/apt/*; }
[ "$(df -BG /tmp | awk 'NR==2{print $4}' | sed 's/G//')" -lt 15 ] && err "Disk yetersiz!"

# ── ÇALIŞMA ──
WORK="/tmp/od-full-$(date +%s)"
...
info "Bağımlılıklar..."
sudo apt update -qq
for pkg in live-build live-config live-boot live-manual; do
    dpkg -l | grep -q "^ii.*$pkg" || sudo apt install -y -qq "$pkg"
done
log "Paketler hazır"
...
# ── HOOK - TÜM ÖZELLEŞTİRMELER ──
info "Özelleştirme hook'u (Kurulum ekranı DAHİL)..."
mkdir -p config/hooks/normal

cat > config/hooks/normal/1000-opendarwin-full.hook.chroot << 'FULLHOOK'
#!/bin/bash
set -e

echo ""
echo "╔══════════════════════════════════════════╗"
echo "║   OpenDarwin 1.0 - TAM Özelleştirme     ║"
echo "║   Boot + Tema + Kurulum Arayüzü         ║"
echo "╚══════════════════════════════════════════╝"
echo ""
...
# 1. LOCALE VE ZAMAN
# ═══════════════════════════════════════════
echo "[01/13] Locale ve zaman dilimi..."
...
if [ -f Pacifico.ttf ]; then
    ...
    echo "  ⚠ Font indirilemedi, devam ediliyor..."
fi
...
/* Beyaz tema - Mavi vurgu - HTML ile aynı    */
/* ═══════════════════════════════════════════ */
...
/* Ana pencere - BEYAZ */
window, .ubiquity, box, notebook, .live-installer {
...
.language-option:hover, .language-item:hover {
    border-color: @accent_color;
    background: @hover_bg;
}

.language-option:checked, .language-item:checked,
.language-option.selected, .language-item.selected {
    border-color: @accent_color;
    background: @selected_bg;
    box-shadow: 0 0 0 3px rgba(0, 113, 227, 0.1);
}
...
.theme-option.auto {
    background: linear-gradient(135deg, #ffffff 50%, @fg_color 50%);
}
...
chmod +x config/hooks/normal/1000-opendarwin-full.hook.chroot
...
START=$(date +%s)
sudo lb build 2>&1 | tee /tmp/build-od-full.log
END=$(date +%s)
...
if [ -f "live-image-amd64.iso" ]; then
    ...
    echo -e "${G}║                                              ║${N}"
    echo -e "${G}║   Dosya: $HOME/opendarwin-1.0-amd64.iso     ║${N}"
    echo -e "${G}║   Boyut: $SZ   Süre: ${MN} dakika             ║${N}"
    echo -e "${G}║                                              ║${N}"
    echo -e "${G}╠══════════════════════════════════════════════╣${N}"
    echo -e "${G}║                                              ║${N}"
    echo -e "${G}║   İNDİR: Sağ tık → Download                  ║${N}"
    echo -e "${G}║                                              ║${N}"
    echo -e "${G}║   KURULUM ADIMLARI:                          ║${N}"
    echo -e "${G}║   1. USB'ye yaz: dd if=iso of=/dev/sdX       ║${N}"
    echo -e "${G}║   2. Boot et                                  ║${N}"
    echo -e "${G}║   3. Kurulumu başlat (Ubiquity)               ║${N}"
